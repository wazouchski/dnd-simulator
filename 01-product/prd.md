...
This PRD defines the requirements for version 1.0 of the D&D Character Balance Tester — a browser-based combat simulation tool that lets players import their D&D Beyond character via a bookmarklet and run turn-by-turn 5e combat encounters to evaluate build performance, survivability, and damage output.

**Character import works via a provided bookmarklet** that the user runs while logged into D&D Beyond. The bookmarklet fetches the character's JSON from the DDB character service API using the user's existing session — no login, scraping, or backend required on our end.
...
## Current UX Flow Direction

The MVP should move away from a single crowded workbench and toward a guided character pipeline:

1. **Character Sheet Upload / Import**
   - Primary action: import from D&D Beyond with a clear bookmarklet copy/install flow.
   - Fallback: paste raw JSON or upload normalized JSON.
2. **Character Sheet Review / Modifications**
   - Show parsed identity, ability scores, AC, HP, speed, weapons, armor, inventory, spells, skills, features, and parser warnings.
   - Let the user correct or confirm sheet-derived loadout decisions before simulation.
3. **Mode Select**
   - Choose **Batch Simulation** for automated odds/results.
   - Choose **Playable Encounter** for an interactive, replayable encounter.
4. **Batch Results**
   - Show aggregate odds, logs, and comparison tools.
5. **Playable Screen**
   - Show character details, HP, AC, movement, skills, spells, weapons, resources, current conditions, and available choices.
   - Make play feel replayable through varied encounter beats, tactical choices, terrain/position prompts, and meaningful spell/skill/resource use.

The product differentiator is reliable full character import plus useful simulation. Other tools can simulate battles; this one must first earn trust by understanding the user's actual sheet.

---

## 🎯 Goals for v1.0

1. **Reliably import a character sheet** from D&D Beyond using the user's own session
2. **Review and correct parsed sheet data** before simulation
3. **Configure and run a combat encounter** against SRD-sourced enemies
4. **Simulate turn-by-turn combat** following 5e RAW action economy where supported
5. **Report aggregate results** across multiple simulation runs
6. **Keep all data local** — no backend, no accounts, no telemetry
...
#### US-101: Import Character via Bookmarklet
> As a **player**, I want to **use a bookmarklet on D&D Beyond to send my character to the simulator** so that **I don't have to manually enter any stats**.

**Acceptance Criteria:**
- [ ] App provides a clearly documented bookmarklet the user drags to their browser's bookmarks bar
- [ ] When run on a DDB character page while logged in, the bookmarklet fetches `character-service.dndbeyond.com/character/v5/character/[ID]` using the user's session cookies
- [ ] The fetched JSON is passed to the simulator app (via URL parameter, localStorage handoff, or direct post to the local app session)
- [ ] Parser extracts: name, class, level, ability scores, proficiency bonus, AC, HP, speed, saving throws, attack actions, and relevant class features
...
- [ ] If the bookmarklet fails (auth error, endpoint unavailable), a descriptive error is shown with troubleshooting steps
- [ ] Successful import shows a character summary card for user confirmation before proceeding
...
| PDF character sheet upload | Deferred to v1.1 as fallback if bookmarklet approach degrades |
| Server-side DDB scraping | Auth wall + ToS violation; bookmarklet runs in user's own session |
...
|---------|------------------|
| v1.1 | Multiclass character support; PDF upload as bookmarklet fallback |
| v1.2 | Full spell simulation (concentration, saves, area effects) |
| v1.3 | Full party simulation (multiple characters vs. encounter) |
| v2.0 | Browser extension (cleaner bookmarklet alternative); homebrew monster entry; save/load history |
